...
public class HomeController : Controller
{
    ...
    public IActionResult Index(ProductsWithFilterVM productsWithFilterVM, int page = 1)
    {
        ...
        {
            product = product.Where(e => e.Price - e.Price * (decimal)(e.Discount / 100.0) >= productsWithFilterVM.MinPrice);
        ...
        {
            product = product.Where(e => e.Price - e.Price * (decimal)(e.Discount / 100.0) <= productsWithFilterVM.MaxPrice);
    ...
    public IActionResult Details(int id)
    {
        ...
        {
            var relatedProduct = _context.Products.Include(e => e.Category).Where(e => e.CategoryId == product.CategoryId && id != e.ProductId).Skip(0).Take(4);

            var topProduct = _context.Products.Include(e=>e.Category).Where(e => e.ProductId != id).OrderByDescending(e => e.Traffic).Skip(0).Take(4);

            var similarProductsName = _context.Products.Include(e => e.Category).Where(e=>e.Name.Contains(product.Name) && id != e.ProductId).Skip(0).Take(4);
